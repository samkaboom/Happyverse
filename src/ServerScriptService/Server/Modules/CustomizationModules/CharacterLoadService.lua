local CharacterLoadService = {}

function CharacterLoadService.LoadCharacter(Player, SlotData, dependencies)

	print("Load Character", Player, SlotData)
	warn(Player.Name, "Save Data amount:", #dependencies.HttpService:JSONEncode(SlotData))
	...
	if tonumber(SlotData["ShirtTemplate"]) then
		local id = dependencies.GetClassicClothingTemplate(SlotData["ShirtTemplate"], "Shirt", "ShirtTemplate")
	...
	if tonumber(SlotData["PantsTemplate"]) then
		local id = dependencies.GetClassicClothingTemplate(SlotData["PantsTemplate"], "Pants", "PantsTemplate")
	...
	local Folder = dependencies.ReplicatedStorage.Info[Player.Name]
	...
	for i = 1, 5, 1 do 
		...
		if dependencies.IsOccupiedSkills(val) then
	...
	Humanoid.BodyTypeScale.Value = dependencies.DefaultType
	Humanoid.BodyProportionScale.Value = dependencies.DefaultProportion
	local boostedHeadScale = (SlotData["Scale"]["Head"] or 1) * dependencies.HeadScaleMultiplier
	...
	for i, Accessory in pairs(Character:GetChildren()) do
		...
		elseif Accessory:GetAttribute("displayAccessory") == true  then
			...
			dependencies.FindToolFromItem(Player, Accessory):Destroy()
			Accessory:Destroy()
		end
	end
	...
	for i, AccessoryTable in pairs(SlotData["Accessories"]) do
		...
		if not AccessoryTable.IsItemPack and not AccessoryTable.IsMeshPart then -- regular accessories
			...
			dependencies.ApplyHeadBoostAccessoryScale(AccessoryTable, NewAccessory, NewMesh)

			if dependencies.MeshIdMatches(AccessoryTable["MeshId"], dependencies.TargetHeadAssetId) then
		...
		elseif AccessoryTable.IsItemPack then -- its an item pack
			...
			local GamepassItemTool, GamepassItemCharacter = dependencies.SpawnGamepassItem:Invoke(Player, AccessoryTable.WeaponName, AccessoryTable.AccessoryWeld.C0, AccessoryTable.Name)
